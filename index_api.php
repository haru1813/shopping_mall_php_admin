...
<?php
    ...
    if($type == "login"){
        ...
        $sql = "SELECT * FROM harumarket_user where haruMarket_user_id='$haruMarket_user_id' and haruMarket_user_pw='$haruMarket_user_pw' and haruMarket_user_role='관리자';";
        $result = mysqli_query($con, $sql);
        $row = mysqli_fetch_assoc($result);

        $total_rows = mysqli_num_rows($result);
        if($total_rows == 0){
            message("비밀번호를 틀리셨거나 관리자 계정이 아닙니다.","500");
            return;
        }
        else{
            // 로그인 인증 처리
